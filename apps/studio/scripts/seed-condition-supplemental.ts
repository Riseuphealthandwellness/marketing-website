/**
 * Backfills supplementalSections and pageLabels onto all condition documents in Sanity.
 *
...
// ─── Helper to generate a stable _key ────────────────────────────────────────
...
type SymptomGroup = {heading: string; items: string[]}

type Section =
  | {type: 'stats'; items: StatItem[]}
  | {type: 'prose'; eyebrow?: string; heading: string; paragraphs: string[]}
  | {type: 'symptoms'; eyebrow?: string; heading: string; description?: string; groups: SymptomGroup[]}
  | {type: 'steps'; eyebrow?: string; heading: string; description?: string; steps: StepItem[]}
  | {type: 'bullets'; eyebrow?: string; heading: string; items: string[]}

function toSanitySection(section: Section) {
  switch (section.type) {
    case 'stats':
      return {
...
// ─── Condition supplemental content ──────────────────────────────────────────

const data: Record<string, {sections: Section[]}> = {
  'alcohol-use-disorder': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Comprehensive initial evaluation', body: 'Your provider reviews your drinking history, past quit attempts, current medications, and overall health. Lab work typically includes liver function tests, a complete metabolic panel, and a blood count.'},
          {title: 'Withdrawal assessment and management', body: 'If your body has become dependent on alcohol, stopping suddenly carries real medical risks — including seizures. Your provider uses the CIWA assessment to gauge withdrawal severity and prescribes appropriate medications if needed for a safer transition.'},
          {title: 'Medication selection', body: 'We discuss which FDA-approved medication fits your situation: naltrexone (oral daily or monthly Vivitrol injection) reduces cravings, acamprosate (Campral) eases the anxiety and restlessness that often drive relapse, and disulfiram (Antabuse) provides a strong deterrent for those who prefer a commitment-based approach.'},
          {title: 'Integrated primary care monitoring', body: 'Liver health, blood pressure, sleep, and nutrition are tracked alongside alcohol use at your regular visits. Problems are addressed as a whole, not in isolation.'},
          {title: 'Ongoing follow-up and adjustment', body: 'Follow-up visits are scheduled at a frequency that matches your situation — more often early in treatment, spacing out as you stabilize. Medications and plans are adjusted based on how you\'re doing.'},
        ],
      },
    ],
  },
  ...
  'opioid-use-disorder': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Initial assessment', body: 'Your provider evaluates your opioid use history, current medications, mental health, and overall physical health. Lab work includes liver and kidney panels and infectious disease screening if indicated.'},
          {title: 'Starting buprenorphine', body: 'Most patients can begin buprenorphine at home. Your provider will give you clear instructions on timing your first dose — typically 12 to 24 hours after your last opioid use, once you\'re beginning to feel withdrawal. This prevents precipitated withdrawal.'},
          {title: 'Dose stabilization', body: 'Over the first two to four weeks, your dose is adjusted until cravings and withdrawal symptoms are well controlled. Follow-up contact is more frequent during this phase.'},
          {title: 'Maintenance treatment', body: 'Ongoing visits — initially every one to four weeks, then monthly as you stabilize — monitor your progress and overall health. MOUD is safe and effective for as long as it\'s needed; there is no required endpoint.'},
          {title: 'Injectable options', body: 'Once you\'re stable on a dose, you may prefer to switch to a monthly injectable formulation (Sublocade or Brixadi) to eliminate daily dosing and simplify your routine.'},
          {title: 'Integrated primary care', body: 'Blood pressure, liver function, mental health, and other conditions are monitored and managed by the same providers — not siloed in a separate office with no knowledge of your history.'},
        ],
      },
    ],
  },
  ...
  'nicotine-use-disorder': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Assessment of use and motivation', body: 'Your provider reviews your smoking history, current level of use, previous quit attempts, and any barriers you\'ve experienced. The Fagerström Test for Nicotine Dependence may be used to gauge physical dependence.'},
          {title: 'Medication selection', body: 'In most cases, varenicline (Chantix) is the first-line choice — it reduces cravings and blocks nicotine\'s reinforcing effects. If varenicline isn\'t appropriate, bupropion or nicotine replacement therapy (patch, gum, or lozenge) may be recommended, and combinations are often used together for higher success rates.'},
          {title: 'Set a quit date', body: 'With varenicline, you typically begin taking the medication one to two weeks before your quit date, giving it time to reach therapeutic levels. With bupropion, the lead time is one to two weeks as well. This structured lead-in dramatically improves outcomes.'},
          {title: 'Support through withdrawal', body: 'Follow-up contact in the first weeks addresses cravings, side effects, and any challenges. Your provider adjusts the plan if needed.'},
          {title: 'Long-term follow-through', body: 'If a slip occurs, it\'s addressed without judgment and treated as information to improve the approach — not a reason to stop treatment. Many people need more than one serious attempt before quitting permanently, and medication significantly improves the odds each time.'},
        ],
      },
    ],
  },
  ...
  'stimulant-use-disorder': {
    sections: [
      ...
      },
      {
        ...
        heading: 'What to expect from treatment',
        steps: [
          {title: 'Medical assessment', body: 'Comprehensive evaluation including cardiovascular exam, blood pressure, lab work, nutritional status, and assessment of any infectious disease risks associated with IV use.'},
          {title: 'Mental health evaluation', body: 'Screening for depression, anxiety, PTSD, and ADHD — all of which frequently co-occur with stimulant use disorder and often require their own treatment. Addressing these conditions significantly improves outcomes.'},
          {title: 'Medication for co-occurring conditions', body: 'While no single medication is FDA-approved for stimulant use disorder itself, medications treating co-occurring depression, anxiety, or ADHD may be appropriate and can reduce the urge to use stimulants.'},
          {title: 'Behavioral support coordination', body: 'Your provider will help coordinate with behavioral health providers and programs that specialize in stimulant use. Contingency management programs, when available, offer the strongest evidence base.'},
          {title: 'Ongoing monitoring and support', body: 'Regular visits track your health recovery — mood, sleep, weight, cardiovascular status, and overall stability. Medical care continues throughout your recovery process.'},
        ],
      },
    ],
  },
  ...
  'benzodiazepine-dependence': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Full evaluation', body: 'Your provider reviews your current medication, dose, how long you\'ve been taking it, any prior withdrawal history, and the underlying condition that led to benzodiazepine use.'},
          {title: 'Converting to a longer-acting agent if needed', body: 'If you\'re on a short-acting benzodiazepine like Xanax or Ativan, your provider may convert your dose to an equivalent amount of diazepam (Valium). Its longer half-life produces a smoother, more gradual decline and makes the taper more manageable.'},
          {title: 'A slow, individualized reduction schedule', body: 'Doses are reduced incrementally — typically 5 to 10 percent of the current dose every two to four weeks. The pace is always yours. If a reduction produces significant symptoms, we hold the current dose or slow the pace.'},
          {title: 'Managing symptoms throughout', body: 'Anxiety, sleep difficulties, and other withdrawal symptoms are actively managed. Non-benzodiazepine medications and other approaches may be used to make the process more tolerable.'},
          {title: 'Treating the underlying condition', body: 'The anxiety, panic, or insomnia that originally led to benzodiazepine use is addressed throughout the taper — not after you\'ve stopped. You will not be left without support for these underlying needs.'},
          {title: 'No pressure, no arbitrary timeline', body: 'Tapering off benzodiazepines can take months to more than a year for long-term users at higher doses. We follow your pace and adjust based on how you\'re doing — not a fixed schedule.'},
        ],
      },
    ],
  },
  ...
  'high-blood-pressure': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Accurate diagnosis', body: 'Blood pressure is measured at every visit, and multiple readings over time are used to confirm a diagnosis. We screen for secondary causes if clinical signs suggest them.'},
          {title: 'Cardiovascular risk assessment', body: 'Blood pressure doesn\'t exist in isolation. Your provider calculates your overall cardiovascular risk, factoring in cholesterol, diabetes, smoking, age, and family history, to guide treatment targets.'},
          {title: 'Lifestyle guidance', body: 'Dietary sodium reduction, the DASH diet, aerobic exercise, weight loss, limiting alcohol, and smoking cessation each have meaningful, quantified effects on blood pressure. We provide specific, practical guidance.'},
          {title: 'Medication management', body: 'If lifestyle changes alone are insufficient, we prescribe from first-line medication classes — ACE inhibitors, ARBs, calcium channel blockers, and thiazide diuretics. Most patients with stage 2 hypertension will need two or more medications to reach target.'},
          {title: 'Home monitoring', body: 'We guide patients on how to monitor blood pressure at home correctly. Home readings are often more accurate than office readings and help us make better-informed decisions between visits.'},
          {title: 'Regular follow-up', body: 'Blood pressure, kidney function, and related conditions are monitored at regular intervals. When blood pressure is well controlled, we adjust the follow-up frequency accordingly.'},
        ],
      },
    ],
  },
  ...
  'type-2-diabetes': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Diagnosis and baseline labs', body: 'A fasting blood glucose, A1c, kidney function panel, cholesterol panel, and urine albumin give us a complete starting picture. We also assess blood pressure and other cardiovascular risk factors.'},
          {title: 'Lifestyle counseling', body: 'Nutrition guidance, physical activity recommendations, and weight management are first-line components of diabetes care. Even modest weight loss (5–10%) significantly improves blood sugar control in most patients.'},
          {title: 'Medication management', body: 'Metformin is the standard first-line medication. For patients who need additional control — or who have cardiovascular or kidney disease — GLP-1 receptor agonists (like Ozempic and Trulicity) and SGLT2 inhibitors (like Jardiance and Farxiga) offer powerful benefits beyond blood sugar reduction, including weight loss, heart protection, and kidney protection.'},
          {title: 'Complication monitoring', body: 'We order A1c every 3–6 months until blood sugar is controlled, then every 6 months. Kidney function, urine protein, and cholesterol are checked annually. We coordinate eye exam and foot exam referrals.'},
          {title: 'Managing related conditions', body: 'Blood pressure and cholesterol are managed with the same focus and rigor as blood sugar, because they drive most of the serious complications of diabetes — heart attack, stroke, and kidney failure.'},
          {title: 'Education and self-management support', body: 'We refer to diabetes educators and registered dietitians for patients who want deeper support. Understanding how food, activity, stress, and illness affect blood sugar empowers patients to manage their condition day to day.'},
        ],
      },
    ],
  },
  ...
  'high-cholesterol': {
    sections: [
      ...
      },
      {
        ...
        heading: 'What to expect from treatment',
        steps: [
          {title: 'Lipid panel blood test', body: 'A fasting lipid panel measures total cholesterol, LDL, HDL, and triglycerides. This is included in routine preventive care and is the foundation for treatment decisions.'},
          {title: 'Cardiovascular risk calculation', body: 'Your provider calculates your 10-year cardiovascular risk using the ASCVD risk score, which incorporates cholesterol, blood pressure, diabetes, smoking, age, and sex. Risk guides treatment intensity.'},
          {title: 'Lifestyle guidance', body: 'The DASH or Mediterranean diet (emphasizing fruits, vegetables, whole grains, and healthy fats), regular aerobic exercise, and weight loss each reduce LDL meaningfully. We provide specific, actionable guidance.'},
          {title: 'Medication when indicated', body: 'Statins — the first-line medication class — reduce LDL by 30–50% and have robust evidence reducing heart attacks and strokes. For patients who cannot tolerate statins, or who need additional LDL reduction, ezetimibe and other agents are available.'},
          {title: 'Follow-up lipid testing', body: 'A repeat lipid panel 4–12 weeks after starting or changing medication confirms the treatment is working. Once controlled, annual monitoring is typical.'},
          {title: 'Integrated primary care', body: 'Cholesterol is managed alongside blood pressure, blood sugar, and other cardiovascular risk factors — since these all interact and collectively determine your heart disease risk.'},
        ],
      },
    ],
  },
  ...
  'hepatitis-c': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'HCV antibody screening', body: 'A simple blood test checks for HCV antibodies. A positive antibody test means past or present infection; a follow-up RNA test confirms active infection.'},
          {title: 'Viral load and genotype testing', body: 'An HCV RNA viral load confirms active infection and establishes a baseline. Genotyping — identifying which of the six HCV strains you have — was historically important for treatment selection, though newer pan-genotypic regimens work across all types.'},
          {title: 'Liver health assessment', body: 'A FIB-4 score (calculated from routine lab values) or FibroScan (non-invasive liver stiffness test) evaluates the degree of liver scarring, which influences treatment urgency and monitoring.'},
          {title: 'Direct-acting antiviral treatment', body: 'Your provider prescribes an 8–12 week course of oral direct-acting antivirals. Common regimens include Harvoni, Epclusa, or Mavyret — all highly effective and generally well tolerated.'},
          {title: 'Cure confirmation', body: 'An HCV RNA test drawn 12 weeks after completing treatment (SVR12) confirms cure. An undetectable viral load at that point means the virus is gone.'},
          {title: 'Ongoing liver health monitoring', body: 'Patients with advanced fibrosis or cirrhosis continue to have regular liver ultrasounds and alpha-fetoprotein testing to screen for liver cancer, even after cure. Patients without cirrhosis who are cured require no ongoing HCV-specific monitoring.'},
        ],
      },
    ],
  },
  ...
  'hiv-and-prep': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Confidential HIV testing', body: 'HIV screening is a standard part of primary care at Rise Up, offered at every new patient visit and annually for patients at ongoing risk. Fourth-generation antigen/antibody tests detect HIV earlier than older tests.'},
          {title: 'PrEP evaluation', body: 'Before starting PrEP, your provider confirms you are HIV-negative, reviews your kidney function (PrEP medications are processed by the kidneys), and screens for active Hepatitis B infection and other STIs.'},
          {title: 'PrEP prescription', body: 'Your provider prescribes the appropriate PrEP regimen — daily oral Truvada or Descovy for most patients, or Apretude (long-acting injectable) for those who prefer every-two-month injections. PrEP is covered by most insurance plans and the manufacturer patient assistance programs.'},
          {title: 'Quarterly PrEP follow-up', body: 'PrEP requires follow-up every three months: HIV testing (to confirm you are still HIV-negative), kidney function, and STI screening. These visits are built into your regular care schedule.'},
          {title: 'Care coordination for people living with HIV', body: 'For patients already living with HIV, Rise Up coordinates with infectious disease specialists to integrate HIV management — antiretroviral therapy, CD4 count, viral load monitoring — with your other primary care needs. Co-occurring OUD and HIV are managed together.'},
        ],
      },
    ],
  },
  ...
  'asthma-and-copd': {
    sections: [
      ...
      },
      {
        ...
        heading: 'What to expect from treatment',
        steps: [
          {title: 'Spirometry and lung function testing', body: 'Spirometry is the gold-standard test for diagnosing and staging both asthma and COPD. It measures how much air you can exhale and how fast. This guides diagnosis and treatment intensity.'},
          {title: 'Inhaler and medication management', body: 'Bronchodilators (short-acting for rescue, long-acting for daily control) and inhaled corticosteroids are the mainstays of both asthma and COPD treatment. We review technique, simplify regimens, and adjust medications based on how controlled your condition is.'},
          {title: 'Smoking cessation support', body: 'Quitting smoking is the single most important intervention for COPD — it slows disease progression significantly. For asthma patients who smoke, cessation also meaningfully improves control. Rise Up provides medication-assisted cessation as part of the same care plan.'},
          {title: 'Vaccinations', body: 'Annual influenza vaccine and pneumococcal vaccines are critical for patients with COPD or asthma — respiratory infections are the leading cause of COPD exacerbations and can be severe in asthma. We keep all vaccinations current.'},
          {title: 'Exacerbation management', body: 'We develop an action plan with every patient so you know what to do if symptoms worsen. We provide access to urgent consultations and coordinate with pulmonology for complex cases or severe COPD.'},
          {title: 'Pulmonary rehabilitation referral', body: 'For moderate to severe COPD, pulmonary rehabilitation — structured exercise and education — significantly improves function, reduces hospitalizations, and improves quality of life. We facilitate referrals when appropriate.'},
        ],
      },
    ],
  },
  ...
  'thyroid-disorders': {
    sections: [
      ...
      },
      {
        ...
        heading: 'What to expect from treatment',
        steps: [
          {title: 'TSH screening', body: 'Thyroid-stimulating hormone (TSH) is the first-line blood test for thyroid function. An elevated TSH suggests hypothyroidism; a suppressed TSH suggests hyperthyroidism. This test is included in routine preventive care.'},
          {title: 'Confirmatory testing', body: 'If TSH is abnormal, free T4 (and free T3 in some cases) are checked to confirm and quantify the dysfunction. Thyroid peroxidase (TPO) antibodies confirm autoimmune Hashimoto\'s disease.'},
          {title: 'Thyroid nodule evaluation', body: 'Thyroid nodules found on physical exam or incidentally on imaging are evaluated with thyroid ultrasound. Size, characteristics, and clinical risk factors determine whether fine-needle aspiration biopsy is needed.'},
          {title: 'Hypothyroidism treatment', body: 'Levothyroxine, a synthetic form of T4, is the standard treatment. Dose is individualized and adjusted based on follow-up TSH testing every 6–8 weeks initially, then annually once stable. Symptoms typically begin improving within a few weeks.'},
          {title: 'Hyperthyroidism management', body: 'Mild or subclinical hyperthyroidism may be monitored closely. More significant cases are managed with antithyroid medications, radioactive iodine, or surgery — usually in coordination with an endocrinologist.'},
          {title: 'Long-term monitoring', body: 'Thyroid function requires periodic monitoring once treatment is established. Changes in weight, other medications, and life events (pregnancy, menopause) can affect thyroid hormone needs, and we adjust accordingly.'},
        ],
      },
    ],
  },
  ...
  'preventive-care-and-wellness-visits': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Comprehensive health review', body: 'Your provider reviews your medical history, current medications, family history, and any symptoms or concerns you\'ve noticed. This is the time to raise anything you\'ve been wondering about.'},
          {title: 'Physical examination', body: 'A head-to-toe physical examination including vital signs, weight and BMI, cardiovascular and lung exam, abdominal exam, skin assessment, and any indicated area based on symptoms or risk factors.'},
          {title: 'Lab work and screenings', body: 'Based on your age, sex, and risk factors, your provider orders the appropriate labs and connects you to the preventive screenings most likely to benefit you.'},
          {title: 'Immunization review', body: 'We review your vaccination history and bring any recommended vaccines up to date at the visit or schedule them.'},
          {title: 'Chronic condition review', body: 'If you have ongoing conditions — diabetes, hypertension, thyroid disease, asthma — the annual visit ensures all the monitoring and medication management for each is coordinated and up to date.'},
          {title: 'Personalized health goals', body: 'We close the visit with a conversation about one or two specific, achievable health goals. Whether it\'s improving diet, increasing activity, quitting tobacco, or improving sleep — we help you build a realistic plan.'},
        ],
      },
    ],
  },
  ...
  'metabolic-syndrome': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Comprehensive metabolic evaluation', body: 'Lab work includes a fasting lipid panel, fasting glucose, HbA1c, liver enzymes, kidney function, and uric acid. Vital signs and waist circumference are measured at every visit. This baseline identifies which of the five components are present and how far outside range they are.'},
          {title: 'Nutrition and lifestyle counseling', body: 'A Mediterranean-pattern or low-glycemic diet is consistently supported by evidence for reducing all five components of metabolic syndrome. Your provider discusses specific changes — reducing refined carbohydrates and added sugars, increasing fiber, healthy fat sources, and sustainable caloric patterns.'},
          {title: 'Physical activity prescription', body: '150 minutes of moderate-intensity aerobic activity per week, combined with resistance training two days per week, is the evidence-based recommendation. Your provider helps build a realistic starting point and a progression plan.'},
          {title: 'Targeted pharmacotherapy', body: 'Depending on which components are out of range and by how much, your provider may prescribe blood pressure medication, a statin, a glucose-lowering agent, or a GLP-1 receptor agonist — medications that address multiple components simultaneously.'},
          {title: 'Ongoing monitoring and adjustment', body: 'Lab work is repeated at 3–6 month intervals to track progress across all components. Treatment intensity is adjusted based on results rather than waiting until a condition crosses an arbitrary threshold.'},
        ],
      },
    ],
  },
  ...
  'glp-1-medications': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Eligibility evaluation', body: 'Your provider reviews your BMI, existing conditions, current medications, and health goals. Lab work typically includes metabolic panel, HbA1c, lipid panel, and thyroid function if indicated.'},
          {title: 'Medication selection', body: 'There are several GLP-1 and dual GIP/GLP-1 options, each with different dosing schedules, approved indications, and insurance coverage profiles. Your provider discusses the best fit based on your clinical picture and insurance situation.'},
          {title: 'Gradual dose titration', body: 'All GLP-1 medications are started at a low dose and increased slowly over several weeks or months. This controlled titration is the primary strategy for minimizing gastrointestinal side effects.'},
          {title: 'Nutritional guidance', body: 'These medications reduce appetite significantly — making it important to prioritize protein and nutrient density within a reduced caloric intake. Your provider discusses dietary patterns that work with, not against, the medication\'s effects.'},
          {title: 'Monitoring and follow-up', body: 'Weight, blood pressure, blood glucose, and tolerability are assessed at each follow-up visit. For patients with diabetes, glucose-lowering medications may need to be adjusted as blood sugar improves. Kidney function and heart rate are monitored over time.'},
          {title: 'Long-term maintenance planning', body: 'GLP-1 medications require ongoing use to maintain weight loss — research shows weight regain after discontinuation is common. Your provider discusses realistic long-term expectations and strategies for maintaining results.'},
        ],
      },
    ],
  },
  ...
  'diabetes-prevention': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Screening and baseline assessment', body: 'A fasting glucose or HbA1c test identifies where you are on the spectrum. If prediabetes is confirmed, we establish a baseline with a full metabolic panel, lipid panel, and blood pressure measurement.'},
          {title: 'Risk stratification', body: 'Your provider calculates your specific 10-year risk of progression to type 2 diabetes using your labs, BMI, family history, and other factors. This helps prioritize the intensity of intervention.'},
          {title: 'Nutrition counseling', body: 'Reducing refined carbohydrates, added sugars, and ultra-processed foods while increasing fiber, vegetables, legumes, and lean protein has a measurable effect on fasting glucose and insulin sensitivity. Your provider discusses realistic, evidence-based dietary changes.'},
          {title: 'Physical activity plan', body: '150 minutes per week of moderate-intensity activity is the DPP-validated target. Even starting with 10–15 minutes daily and progressing from there produces meaningful improvement in insulin sensitivity.'},
          {title: 'Metformin (when indicated)', body: 'For patients who cannot achieve lifestyle targets or who are at very high risk (HbA1c near 6.4%, strong family history, obesity), metformin is an effective, safe, and low-cost medication that reduces diabetes risk by 31% in the DPP. Your provider discusses whether this is appropriate for your situation.'},
          {title: 'Annual rescreening', body: 'HbA1c or fasting glucose is repeated annually to track progress. We celebrate improvement and adjust the plan if things aren\'t moving in the right direction.'},
        ],
      },
    ],
  },
  ...
  'medically-supervised-weight-loss': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Comprehensive baseline evaluation', body: 'Your provider takes a thorough medical history, measures height, weight, waist circumference, blood pressure, and BMI, and reviews your medications. Lab work screens for thyroid dysfunction, insulin resistance, metabolic abnormalities, and other contributors to weight gain.'},
          {title: 'Goal setting and expectations', body: 'We discuss realistic, health-focused goals. A 5–10% reduction in starting weight produces clinically meaningful improvements in blood sugar, blood pressure, cholesterol, and joint burden — often more meaningful than achieving a specific target weight.'},
          {title: 'Nutrition guidance', body: 'Your provider discusses evidence-based dietary approaches appropriate for your conditions and preferences — a consistent calorie reduction, a lower-glycemic approach, or a Mediterranean-style pattern. We emphasize sustainable change over restrictive short-term protocols.'},
          {title: 'Physical activity integration', body: 'Even modest increases in physical activity improve metabolic health independently of weight loss. Your provider builds a realistic activity prescription starting from where you are now.'},
          {title: 'Pharmacotherapy evaluation', body: 'For patients who are appropriate candidates, FDA-approved weight loss medications — including GLP-1 receptor agonists, phentermine/topiramate, bupropion/naltrexone, or others — are discussed. These are tools, not shortcuts, and work best when combined with lifestyle change.'},
          {title: 'Regular monitoring and adjustment', body: 'Weight, labs, blood pressure, and relevant comorbidity markers are assessed at each follow-up visit. The plan evolves based on your progress and response.'},
        ],
      },
    ],
  },
  ...
  'obesity': {
    sections: [
      ...
      },
      {
        ...
        steps: [
          {title: 'Medical evaluation', body: 'A thorough initial evaluation includes height, weight, waist circumference, blood pressure, BMI classification, and a targeted review of weight-related comorbidities. Lab work screens for thyroid function, metabolic abnormalities, blood glucose, liver enzymes, kidney function, and lipids.'},
          {title: 'Goal framing', body: "Before discussing intervention, your provider discusses what 'success' looks like for you — which is not always a specific weight. A 5–10% reduction in starting weight produces measurable clinical benefit: lower blood pressure, improved blood sugar, reduced joint load, better sleep. Framing goals around health outcomes, not a target weight, improves adherence and outcomes."},
          {title: 'Nutrition and behavior counseling', body: 'Evidence-based dietary approaches — including a moderate caloric deficit, reduced ultra-processed foods, increased protein and fiber — are discussed in the context of your current diet, preferences, and life circumstances. We address the psychological and behavioral dimensions of eating alongside the nutritional ones.'},
          {title: 'Physical activity prescription', body: '150–300 minutes of moderate aerobic activity per week plus resistance training are the evidence-based targets. Your provider builds an individualized starting point, accounting for musculoskeletal limitations, fitness level, and preferences.'},
          ...
          {title: 'Long-term management', body: 'Obesity is a chronic condition requiring long-term management, not a one-time intervention. Your care plan includes regular follow-up, annual labs, and adjustment of pharmacotherapy as needed. We coordinate with the management of comorbidities so that as weight improves, medications for related conditions can be appropriately adjusted.'},
        ],
      },
    ],
  },
  ...
  'sexually-transmitted-infections': {
    sections: [
      ...
      },
      {
        ...
        heading: 'What to expect',
        steps: [
          {title: 'Confidential risk assessment', body: 'Your provider asks about your sexual health history in a private, non-judgmental setting. This helps determine which tests are most relevant for you.'},
          {title: 'Testing panel', body: 'Based on your history and risk factors, your provider orders appropriate tests — which may include urine or swab samples for chlamydia/gonorrhea, blood tests for syphilis, HIV, and Hepatitis B and C, and other targeted tests.'},
          {title: 'Treatment of positive results', body: 'Bacterial STIs (chlamydia, gonorrhea, syphilis) are treated with antibiotics, often a single dose or short course. Your provider explains the treatment plan and any sexual activity restrictions during treatment.'},
          {title: 'Test of cure', body: 'Gonorrhea and chlamydia require re-testing 3 months after treatment to check for reinfection — not just cure confirmation. This is a routine part of the follow-up plan.'},
          {title: 'Partner notification support', body: 'Your provider discusses partner notification — informing recent sexual partners that they may need testing. We can help with anonymous partner notification resources if needed.'},
          {title: 'PrEP and prevention', body: 'If your testing reveals HIV risk factors, we discuss PrEP (HIV prevention medication) and other prevention strategies — including consistent condom use and harm reduction for people who use drugs.'},
        ],
      },
    ],
  },
...
// ─── Default page labels applied to every condition ──────────────────────────

const defaultPageLabels = {
  eyebrow: 'Conditions',
  ctaHeading: 'Ready to get started?',
  ctaDescription: 'Our team can help you understand your options and take the next step.',
  ctaButtonLabel: 'Start your care',
  ctaButtonHref: '/new-patients',
  medicationsHeading: 'Medications & treatments',
...
async function run() {
  console.log(`Seeding supplementalSections + pageLabels for ${Object.keys(data).length} conditions…`)

  for (const [slug, {sections}] of Object.entries(data)) {
    const doc = await client.fetch<{_id: string} | null>(
      `*[_type == "condition" && slug.current == $slug][0]{ _id }`,
      {slug},
    )

    if (!doc) {
      console.warn(`  ⚠️  No condition found for slug "${slug}" — skipping`)
      continue
    }

    const supplementalSections = sections.map(toSanitySection)

    await client.patch(doc._id).set({supplementalSections, pageLabels: defaultPageLabels}).commit()
    console.log(`  ✓  ${slug}`)
  }
